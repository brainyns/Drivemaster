...
function VentaDetalle({ id, onVolver, token }) {
  ...
  const handleImprimir = async () => {
    setDescargando(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8080'}/api/ventas/${id}/pdf`, {
      ...
      <div className="vd-body">
        ...
        <div className="vd-left">
          ...
          <div className="vd-panel relative">
            ...
            {venta.fechaEnvio && (
              <p className="vd-resumen-fecha-label" style={{ marginTop: ".75rem" }}>Enviado</p>
            )}
            {venta.fechaEnvio && (
              <p className="vd-resumen-fecha">
                {new Date(venta.fechaEnvio).toLocaleDateString("es-CO", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}
              </p>
            )}
            {venta.fechaEntrega && (
              <p className="vd-resumen-fecha-label" style={{ marginTop: ".75rem" }}>Entregado</p>
            )}
            {venta.fechaEntrega && (
              <p className="vd-resumen-fecha">
                {new Date(venta.fechaEntrega).toLocaleDateString("es-CO", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}
              </p>
            )}
